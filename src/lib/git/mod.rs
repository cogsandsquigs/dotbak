...
impl Dotbak {
    ...
    pub fn clone_repo<P, U>(path: P, url: Url) -> Result<Repository>
    where
        P: AsRef<Path>,
    {
        let path = path.as_ref();
        ...
        let mut prepare_clone = gix::prepare_clone(url.clone(), path)?; // TODO: get rid of clone.
